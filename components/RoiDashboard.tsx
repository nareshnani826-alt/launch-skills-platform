  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><h2 className="text-xl font-semibold">Certification ROI Dashboard</h2><p className="text-sm text-neutral-500">Track investment, opportunity influence, and capability unlocks by quarter.</p></div>
        <div className="flex flex-wrap gap-2"><label className="text-sm text-neutral-600">Quarter <select className="ml-1 rounded border border-neutral-300 bg-white px-2 py-1.5" value={props.selectedQuarter} onChange={(event) => selectQuarter(event.target.value)}><option value="all">All quarters</option>{props.quarters.map((quarter) => <option key={quarter} value={quarter}>{quarter}</option>)}</select></label><a className="rounded bg-accent px-3 py-1.5 text-sm font-medium text-white hover:opacity-90" href={`/api/admin/roi/export?quarter=${encodeURIComponent(props.selectedQuarter)}`}>Export CSV</a></div>
      </div>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">{[[money(props.totals.totalSpend), "Training investment"], [money(props.totals.totalRevenue), "Revenue influenced"], [props.totals.multiple ? `${props.totals.multiple.toFixed(1)}x` : "—", "Blended ROI"], [money(props.totals.linkedRevenue), "Opportunity-linked"], [money(props.totals.capabilityRevenue), "Capability unlock"]].map(([value, label]) => <div className="card" key={label}><div className="text-2xl font-semibold">{value}</div><div className="text-xs text-neutral-500">{label}</div></div>)}</div>
